...
//     CreateTree::Instance()->opPhoton_time_det_eff[10].push_back( theTrack->GetGlobalTime()/nanosecond + );
...
//       cout << " tracing optical photons ... " << endl;
      ...
      if (theTrack->GetVolume()->GetName() == "EE_phys" && (nStep == 1) ) CreateTree::Instance()->opPhoton_n ++;
      
      // extracted photons
      if (thePrePVName == "EE_phys" && thePostPVName == "Grease_phys" ) CreateTree::Instance()->opPhoton_n_ext ++;
  ...
  else
  {
    ...
    {	   
     ...
     double par0 = 4.42145e-02, par1 = 3.04950e+01, par2 = 1.60709e-04;
     
//      cout << " att coeff = " <<  fAtt(par0, par1, par2, pos[2])/par0 << endl;
     for (int j = 0; j < 10; j++) {
	 CreateTree::Instance()->Total_ion_energy_att[j]    += ion_energy* fAtt(par0, par1, par2, pos[2] + 110) /par0;
     }
