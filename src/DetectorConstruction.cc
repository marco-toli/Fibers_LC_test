...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  G4Tubs* Grease_solid = new G4Tubs("Grease_solid", 0, win_r, win_l/2, startAngle, spanningAngle);
  G4Tubs* Win_solid = new G4Tubs("Win_solid", 0, win_r, win_l/2, startAngle, spanningAngle);
  ...
  // physical: placement
  G4VPhysicalVolume* EE_phys[25];
  G4VPhysicalVolume* EE_phys_alveolar[25];

  G4VPhysicalVolume* Grease_rear_phys[25];
  G4VPhysicalVolume* Win_rear_phys[25];
  
  G4VPhysicalVolume* Det_layer_rear_phys[25];
  G4VPhysicalVolume* Det_rear_phys[25];
  ...
  G4double x_pos_pmt[25];
  G4double y_pos_pmt[25];
  ...
  for (int iX = 0; iX < 5; iX ++){
    for (int iY = 0; iY < 5; iY ++){
      ...
      iCrystal = 5*iX + iY;
      x_pos[iCrystal] = iX*(rear_face_size+front_face_size + 2*alveola_thickness)/2.;	// position the baricenter of crystals and then rotating them by
      y_pos[iCrystal] = iY*(rear_face_size+front_face_size + 2*alveola_thickness)/2.;
      
      x_pos_pmt[iCrystal] = iX*(rear_face_size*2 + 2*alveola_thickness)/2.;	// position the baricenter of crystals and then rotating them by
      y_pos_pmt[iCrystal] = iY*(rear_face_size*2 + 2*alveola_thickness)/2.;

      G4RotationMatrix* piRot = new G4RotationMatrix;
      piRot->rotateY(-pointingAngle*rad*(iX + 0.5));
      piRot->rotateX(pointingAngle*rad*(iY + 0.5));
...
//       if (iX == 0)
      ...
      EE_phys[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos[iCrystal], y_pos[iCrystal], 0), EE_log, name, expHall_log, false, 0);
      
      sprintf(name, "EE_phys_alveolar_%d", iCrystal);
      EE_phys_alveolar[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos[iCrystal], y_pos[iCrystal], 0), EE_log_alveolar, name, expHall_log, false, 0);

      sprintf(name,"Grease_rear_%d",iCrystal);
      Grease_rear_phys[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos_pmt[iCrystal], y_pos_pmt[iCrystal], (crystal_length + win_l)/2), Grease_log, name, expHall_log, false, 0);
      
      sprintf(name,"Win_rear_%d",iCrystal);
      Win_rear_phys[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos_pmt[iCrystal], y_pos_pmt[iCrystal], (crystal_length + win_l)/2 + win_l), Win_log, name, expHall_log, false, 0);

      sprintf(name,"Det_layer_rear_%d", iCrystal);
      Det_layer_rear_phys[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos_pmt[iCrystal], y_pos_pmt[iCrystal], (crystal_length + depth)/2 + 2*win_l) , Det_layer_log, name, expHall_log, false, 0);

      sprintf(name,"Det_rear_%d", iCrystal);
      Det_rear_phys[iCrystal] = new G4PVPlacement(piRot, G4ThreeVector(x_pos_pmt[iCrystal], y_pos_pmt[iCrystal], (crystal_length  + det_d)/2 + 2*win_l + depth) , Det_log, name, expHall_log, false, 0);
    }
  }
	...
	for (int iX = 0; iX < 5; iX ++){
	  for (int iY = 0; iY < 5; iY ++){
	    ...
	    CrystalSurfaceSkin   = new G4LogicalBorderSurface("Lateral", EE_phys[iCrystal], EE_phys_alveolar[iCrystal], OpCrystalSurface);
	  }
	}
